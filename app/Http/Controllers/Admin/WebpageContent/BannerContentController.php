...
class BannerContentController extends Controller
{
    ...
    {
        $formData = $request->except('_token', 'banner_id','edit_bannerimage','edit_employerbannerimage','edit_companyimage');
        if ($formData['banner_title'] == '' || $formData['banner_description'] == '' || $formData['employer_banner_title'] == ''
        || $formData['employer_banner_description'] == '') {
        ...
        if ($request->hasFile('employer_banner_image')) {
            $file = $request->file('employer_banner_image');
            $destinationPath = public_path('uploads/homepage/employer');
            $fileName = $file->getClientOriginalName();
            $fileExtension = explode('.', $fileName);
            ...
            $file->move($destinationPath, $fileName);
            $formData['employer_banner_image'] = $fileName;
        } else {
            $formData['employer_banner_image'] =  $request->input('edit_employerbannerimage');
        }
