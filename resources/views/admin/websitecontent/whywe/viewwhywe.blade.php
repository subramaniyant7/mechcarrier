...
@section('content')
    ...
    <div class="page-meta">
        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Why We</li>
            </ol>
            <a href="{{ route('managewhywe') }}" class="btn btn-primary btn-rounded mb-2 me-4"><i class="fa-solid fa-plus"></i> Why We</a>
        </nav>
    </div>
    <!-- /BREADCRUMB -->
    ...
    <div class="row layout-top-spacing">
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
            @php
                $title = getMixedContentByType('whywe');
            @endphp
            <form action="{{route('savemixedcontent')}}" method="POST" autocomplete="off">
                ...
                <div class="form-group row  mb-4">
                    ...
                    <input type="hidden" class="form-control form-control" name="mixed_content_type" value="whywe">
                    <div class="col-sm-4">
            ...
            <div class="widget-content widget-content-area br-8">
                <table id="zero-config" class="table table-striped dt-table-hover" style="width:100%">
                    ...
                    <tbody>
                    @forelse($whywe as $k => $whywe)
                        @php
                            $class = $whywe->status == 1 ? 'badge-light-success' : '';
                        @endphp
                        <tr>
                            <td> {{ $k+1 }} </td>
                            <td> {{ $whywe->whywe_name }} </td>
                            <td><img style="height: 100px;" src="{{ URL::asset('uploads/whywe/'.$whywe->whywe_image)}}" alt="Logo"></td>
                            <td> {{ $whywe->whywe_position }} </td>
                            <td>  <span class="{{ statusClass()[$whywe->status-1] }}">{{ statustype()[$whywe->status-1] }}</span> </td>
                            <td>
                                <div class="action-btns">
                                    <a href="{{ route('actionwhywe',['option' => 'edit', 'id' => encryption($whywe->whywe_id)])  }}" class="action-btn btn-edit bs-tooltip me-2" data-toggle="tooltip" data-placement="top" title="Edit">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
                                    </a>
                                    <a href="{{ route('actionwhywe',['option' => 'delete', 'id' => encryption($whywe->whywe_id)]) }}" class="action-btn btn-delete bs-tooltip" data-toggle="tooltip" data-placement="top" title="Delete">
