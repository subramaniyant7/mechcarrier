...
@section('content')
    ...
    <div class="page-meta">
        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
            ...
            <a href="{{ route('manageeducation') }}" class="btn btn-primary btn-rounded mb-2 me-4"><i
                    class="fa-solid fa-plus"></i> Education</a>
        </nav>
    </div>
    <!-- /BREADCRUMB -->
        ...
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
            ...
            <div class="widget-content widget-content-area br-8">
                <table id="zero-config" class="table table-striped dt-table-hover" style="width:100%">
                    ...
                    <tbody>
                        @forelse($education as $k => $education)
                            @php
                                $class = $education->status == 1 ? 'badge-light-success' : '';
                            @endphp
                            <tr>
                                <td> {{ $k + 1 }} </td>
                                <td> {{ $education->education_name }} </td>
                                <td> <span
                                        class="{{ statusClass()[$education->status - 1] }}">{{ statustype()[$education->status - 1] }}</span>
                                </td>
                                <td>
                                    <div class="action-btns">
                                        <a href="{{ route('actioneducation', ['option' => 'edit', 'id' => encryption($education->education_id)]) }}"
                                        ...
                                        <a href="{{ route('actioneducation', ['option' => 'delete', 'id' => encryption($education->education_id)]) }}"
