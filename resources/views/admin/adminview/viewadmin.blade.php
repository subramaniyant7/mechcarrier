...
@section('content')
    ...
    <div class="page-meta">
        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
            ...
            <a href="{{ route('manageadmin') }}" class="btn btn-primary btn-rounded mb-2 me-4"><i class="fa-solid fa-plus"></i> Admin</a>
        </nav>
    </div>
    <!-- /BREADCRUMB -->
                ...
                <table id="zero-config" class="table table-striped dt-table-hover" style="width:100%">
                    ...
                    <tbody>
                        @forelse($admindetails as $k => $admin)
                            @php
                                $class = $admin->status == 1 ? 'badge-light-success' : '';
                            @endphp
                            <tr>
                                <td> {{ $k+1 }} </td>
                                ...
                                <td>   <span class="{{ statusClass()[$admin->status-1] }}">{{ statustype()[$admin->status-1] }}</span> </td>
                                <td>
                                    <div class="action-btns">
                                        <a href="{{ route('actionadmin',['option' => 'edit', 'id' => encryption($admin->admin_id)])  }}" class="action-btn btn-edit bs-tooltip me-2" data-toggle="tooltip" data-placement="top" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
                                        </a>
                                        <a href="{{ route('actionadmin',['option' => 'delete', 'id' => encryption($admin->admin_id)]) }}" class="action-btn btn-delete bs-tooltip" data-toggle="tooltip" data-placement="top" title="Delete">
