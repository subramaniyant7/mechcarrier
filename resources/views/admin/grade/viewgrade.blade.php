...
@section('content')
    ...
    <div class="page-meta">
        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
            ...
            <a href="{{ route('managegrade') }}" class="btn btn-primary btn-rounded mb-2 me-4"><i
                    class="fa-solid fa-plus"></i> Grade</a>
        </nav>
    </div>
    <!-- /BREADCRUMB -->
        ...
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
            ...
            <div class="widget-content widget-content-area br-8">
                <table id="zero-config" class="table table-striped dt-table-hover" style="width:100%">
                    ...
                    <tbody>
                        @forelse($grade as $k => $grade)
                            @php
                                $class = $grade->status == 1 ? 'badge-light-success' : '';
                                $courseData = App\Http\Controllers\Admin\Helper\CommonHelperController::getEducation($grade->education_id);
                                $courseName = '';
                                if (count($courseData)) {
                            ...
                            <tr>
                                <td> {{ $k + 1 }} </td>
                                <td> {{ $courseName }} </td>
                                <td> {{ $grade->education_grade_name }} </td>
                                <td> <span
                                        class="{{ statusClass()[$grade->status - 1] }}">{{ statustype()[$grade->status - 1] }}</span>
                                </td>
                                <td>
                                    <div class="action-btns">
                                        <a href="{{ route('actiongrade', ['option' => 'edit', 'id' => encryption($grade->education_grade_id)]) }}"
                                        ...
                                        <a href="{{ route('actiongrade', ['option' => 'delete', 'id' => encryption($grade->education_grade_id)]) }}"
