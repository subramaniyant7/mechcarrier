...
@php
    ...
    if ($type == 'edit') {
        ...
        $allIndia = $data[0]->user_preferred_state == 0;
        $total = count($stateId);
    ...
    @for ($l = 0; $l < $total; $l++)
        @php
            if ($type == 'edit') {
                ...
                if ($data[0]->user_preferred_city != '') {
                    $cityInfo = \App\Http\Controllers\Frontend\Helper\HelperController::getCityById($cityId[$l]);
                    if (count($cityInfo)) {
        ...
        @if ($l == 0)
            <div class="preferred_main" data-id="{{ $l + 1 }}">
                <div class="row">
                    <div class="col-md-4">
                        <div class="d-flex">
        ...
        @endif
        @if ($l + 1 == $total || $total == 1)
            </div>
        @endif
    @endfor

    <div class="add_more" style="text-align: left;display:{{ $total > 2 || $allIndia ? 'none' : 'block' }}"><span
            style="color: #1D56BB;cursor:pointer;position:unset;font-size:16px;">Add More</span>
    </div>
